...
as
    select listing.title, listing .description , listing .author, listing ."timestamp", listing .cancelled, listing .nonce, listing .sold, listing .token_price , array_agg(distinct (a.nft_contract)), SUM(a.nft_amount) as nft_quantity, count(a) as nft_distincts, CASE WHEN count(a) = 0 THEN ARRAY[]::json[] ELSE array_agg(a.nfts) END AS nfts
